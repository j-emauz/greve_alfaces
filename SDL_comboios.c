...
void SDL_escreve_linhas(FERROVIA *todas[],int cores[][DIMrgb]){
	...
	for(i=0; i<MAX&&todas[i]!=NULL; ++i){
		...
		for(head = todas[i];head!=NULL && strcmp(lident, head->lident)==0; head=head->RA){
			...
			filledCircleRGBA(g_pRenderer, head->pont.coord[coordX], head->pont.coord[coordY], 5, cores[head->pont.cor][R], cores[head->pont.cor][G], cores[head->pont.cor][B], cores[head->pont.cor][ALPA]);
             SDL_RenderPresent(g_pRenderer);
             SDL_Delay(10);
